...
<a href="#modalCreate" data-toggle="modal" class="btn btn-info">Create Product Type</a>

<table class="table">
    <thead>
        <tr>
            <th>Nama Tipe Product</th>
            <th>Created At</th>
            <th>Updated At</th>
            <th>Edit</th>
            <th>Delete</th>
    ...
    <tbody>
        ...
        <tr id="tr_{{ $d->id }}">
            ...
            <td>{{ $d->updated_at }}</td>
            <td>
                <a class="btn btn-warning" href="{{ route('producttype.edit', $d->id) }}">Edit</a>
            </td>
            <td>
                <form method="POST" action="{{route('producttype.destroy', $d->id) }}">
                    @csrf
                    @method('DELETE')
                    <input type="submit" value="Delete Product Type" class="btn btn-danger"
